...
millones de distancias en 6 dimensiones por iteración. El valor de $K$ se fijará en PC2
comparando la inercia y el coeficiente de silueta para varios valores sobre una muestra, y la
inicialización será k-means++ con semilla fija, la misma para las versiones secuencial y concurrente.
...
El caso es la propuesta del grupo; el enunciado prevé que el docente asigne formalmente el caso y el
modelo. Al cierre de este segundo informe la asignación formal sigue pendiente y el equipo continúa
de forma provisional (Sección~\ref{sec:correcciones}).
...
El dataset cubre un solo mes, y eso limita lo que los clusters pueden decir. Enero de 2024 incluye
dos feriados, Año Nuevo (lunes 1) y el Día de Martin Luther King (lunes 15), además del invierno.
Las features de hora y día de la semana capturan parte de esa variación, pero un feriado se confunde
con un día laborable del mismo día de la semana. Los tipos de viaje que encontremos describen enero
de 2024; comprobar si se repiten en otro mes queda como validación para el TP.

\subsection{Diseño concurrente de referencia}\label{sec:diseno}

\emph{Esta subsección se conserva como se entregó en la PC1. Lo que cambió al implementarla, y los
compromisos que se retiran, están en la Sección~\ref{sec:correcciones}; el algoritmo implementado es
el Algoritmo~\ref{alg:lloyd-chunks} de la Sección~\ref{sec:promela}.}
...
\centering
\small
\input{secciones/figuras/workerpool}
\end{figure}
...
Cuarto, la parte serial pone techo al speedup. Según la ley de \textcite{amdahl1967}, si
...
serial, con 8 goroutines el speedup sería de unas 4 veces, y ni con infinitas goroutines pasaría de unas 7.
...
(más viajes en el mismo tiempo al sumar goroutines), que describe la ley de \textcite{gustafson1988}. La memoria, en cambio, no es el límite: las 6 features de 2,8 millones
