...
\begin{itemize}
  ...
  \item criterio de parada $\max_j \lVert \mu_j^{(t)} - \mu_j^{(t-1)} \rVert_\infty \le
...
correcto y sobre el mutante (Spin 6.5.2, búsqueda exhaustiva completa, sin reducción por
\emph{bitstate}). El modelo correcto termina con \texttt{errors: 0} tras recorrer 2\,407 estados; el
mutante viola la aserción \texttt{compartido == 4} a profundidad 205, en la segunda iteración. El
contraejemplo se reproduce paso a paso con \texttt{spin -t -p -DMUTANTE kmeans.pml}, y el
\texttt{Makefile} de \texttt{tp/spin/} comprueba en cada corrida que los dos números sigan siendo
cero y uno, para que el modelo no deje de probar lo que dice probar sin que nadie lo note.
