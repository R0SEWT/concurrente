...
% ========================================================
\section{Investigación bibliográfica}\label{sec:temas}
...
\textbf{Responsable:} \nombreTercero. Las opiniones críticas y la conclusión de este tema las
redactó \nombreRody, porque el integrante asignado no las entregó.
...
\tarjeta{nigro2022}{K-means paralelo en Java}{
  ...
  integrante={\nombreRody},
  opinion={De los tres trabajos del tema, este es el que más se parece a lo que haremos en
    Go. El beneficio de la concurrencia es claro: con 16 hilos logra un speedup cercano a 11 y
    reproduce exactamente los centroides y las iteraciones de la versión secuencial, que es el criterio
    de corrección que adoptamos. Pero 11 sobre 16 es una eficiencia cercana al 69\,\%: casi un tercio
    del potencial se pierde en partes seriales y en coordinar hilos, algo que la ley de Amdahl ayuda a
    explicar. También me deja una duda sobre el control: con \textit{parallel streams} el runtime decide
    el particionado, cómodo pero opaco, mientras que en Go nosotros elegimos el tamaño de bloque y
    podemos medirlo. Por último, sus puntos tienen 68 dimensiones y los nuestros 6; con menos cálculo
    por viaje, sospecho que el costo de coordinar pesará más en proporción y nuestra eficiencia será
    menor.},
}
...
\tarjeta{kwedlo2021}{K-means acelerado en memoria compartida}{
  ...
  aporte={En 24 núcleos, con 6 datasets sintéticos (hasta $10^9$ puntos) y 7 reales, alcanza una
    ...
    dimensión.},
  integrante={\nombreRody},
  opinion={Me interesa porque ataca un problema que en Lloyd no existe: con poda, cada punto
    cuesta distinto, y repartir bloques iguales ya no reparte trabajo igual. Crear una tarea por llamada
    recursiva adapta la granularidad a la geometría de los datos, y el resultado sigue siendo exacto, lo
    que es un beneficio real. El perjuicio lo reconocen los propios autores: a medida que el algoritmo
    avanza, cada tarea hace menos trabajo, pero el costo de gestionar el \textit{pool} de tareas no baja
    y puede limitar la escalabilidad. En Go las goroutines son baratas, pero no gratis, así que abrir una
    por cada subárbol diminuto repetiría el problema; habría que fijar un umbral por debajo del cual se
    recorre en serie. También noto que la ventaja cae al subir la dimensión y que construir el kd-tree
    agrega memoria y preprocesamiento. Con nuestras 6 dimensiones la idea encaja, pero la dejaría para
    después de medir el Lloyd concurrente simple.},
}
...
\tarjeta{kwedlo2024}{IMM paralelo en memoria compartida}{
  ...
  integrante={\nombreRody},
  opinion={Este trabajo muestra, mejor que los otros dos, que el nivel en que se abre el
    paralelismo importa tanto como la cantidad de hilos: el mismo algoritmo, con paralelismo anidado,
    pasa a ser entre 1,3 y 15 veces más rápido que la re-implementación de referencia. El beneficio,
    sin embargo, depende de la dimensión del espacio de features, y la evaluación usa solo 9 datasets
    sintéticos, así que no sé cuánto de esa ganancia se sostendría con datos reales. El paralelismo
    anidado también tiene un costo: si no se controla, más hilos compiten por los mismos núcleos.
    Además, el artículo no acelera K-means en sí, sino la explicación de sus clusters. Eso no entra en
    PC2, pero me parece útil después: describir cada tipo de viaje con cortes simples de hora, duración
    o distancia haría más legible el análisis por zona.},
}
...
\tarjeta{bellavita2026}{Kernel K-means con poca comunicación entre GPUs}{
  ...
  integrante={\nombreDayana},
  opinion={En este trabajo, el aporte central de la programación concurrente no está
    únicamente en distribuir el cálculo de Kernel K-means entre varias GPUs,
...
\tarjeta{xu2021}{workers de respaldo dinámicos}{
  ...
  integrante={\nombreDayana},
  opinion={Para mí, lo que aporta la programación concurrente en este trabajo es reducir el
    impacto de los retrasos dentro de los sistemas distribuidos de
