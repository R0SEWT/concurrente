...
\tarjeta{li2023}{K-means a gran escala en GPU}{
  ...
  propuesta={ASB K-means (\textit{Asynchronous Selective Batched}) procesa por lotes y usa la
    desigualdad triangular para omitir, en cada iteración, los puntos cuya asignación no puede
    cambiar. Así reduce los datos que se transfieren.},
  algoritmo={\paso{1}Mantener cotas de distancia de cada punto a los centroides.
    \paso{2}Tras mover los centroides, actualizar las cotas y marcar los puntos que podrían cambiar
    de cluster.
    \paso{3}Enviar a la GPU solo esos candidatos, en lotes asíncronos.
    \paso{4}Reasignarlos, recalcular los centroides y repetir.},
  stack={CPU y GPU con CUDA, lotes asíncronos; comparación con el K-means de RAPIDS cuML.},
  aporte={Hasta 15 veces más rápido que un K-means estándar en GPU, y más rápido que RAPIDS cuML en
    todos los datasets probados.},
  integrante={\nombreRody},
}
...
mide algo distinto (Tabla~\ref{tab:sintesis3}). ASB acelera sin perder exactitud. Voevodski cambia
...
\textcite{li2023} & Distancias y transferencias CPU--GPU & Exacto
  & Hasta 15 veces frente a K-means estándar en GPU \\
\addlinespace
